...
model_name = "pcnn_lr:{:.5f}_nr-resnet{}_nr-filters{}".format(
    args.lr, args.nr_resnet, args.nr_filters
)
    ...
    for epoch in range(args.max_epochs):
        ...
        for batch_idx, (input, _) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % args.print_every == 0:
                deno = args.print_every * args.batch_size * np.prod(obs) * np.log(2.0)
                writer.add_scalar("train/bpd", (train_loss / deno), writes)
                print(
                    "loss : {:.4f}, time : {:.4f}".format(
        ...
        deno = batch_idx * args.batch_size * np.prod(obs) * np.log(2.0)
        ...
        if (epoch + 1) % args.save_interval == 0:
            ...
            tutils.save_image(
                sample_t,
                "images/{}_{}.png".format(model_name, epoch),
                nrow=5,
                padding=0,
            )
