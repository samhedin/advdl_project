...
def discretized_mix_logistic_loss(x, l):
    ...
    x = x.unsqueeze(-1) + Variable(
        torch.zeros(xs + [nr_mix]).cuda(), requires_grad=False
    )
...
def mix_logistic_loss(x, l, likelihood=False):
    ...
    x = x.contiguous()
    x = (
        x.unsqueeze(-1) + (torch.zeros(xs + [nr_mix]).cuda()).detach()
    )  # [B, 32, 32, 3, nr_mix]
    m2 = (means[:, :, :, 1, :] + coeffs[:, :, :, 0, :] * x[:, :, :, 0, :]).view(
        xs[0], xs[1], xs[2], 1, nr_mix
    )  # [B, 32, 32, 1, nr_mix]
    ...
    means = torch.cat(
        (means[:, :, :, 0, :].unsqueeze(3), m2, m3), dim=3
    )  # [B, 32, 32, 3, nr_mix]
    ...
    log_probs = mid_in - log_scales - 2.0 * F.softplus(mid_in)  # [B, 32, 32, 3, nr_mix]

    if likelihood:
        log_probs = torch.sum(log_probs, dim=3) + log_prob_from_logits(logit_probs)
        return log_sum_exp(log_probs)

    log_probs = torch.sum(log_probs, dim=3) + log_prob_from_logits(
        logit_probs
    )  # [B, 32, 32, nr_mix]
